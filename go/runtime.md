...
在调度器初始化完成后并不会立即执行runtime.main，而是在后续的mstart调用中被调度器调度执行。在这个过程中，只会将runtime.main的入口地址压栈，进而将其传递给newproc进行使用，而后newproc完成G的创建保存到G的运行现场中，因此真正执行会等到mstart后才会被调度执行。
