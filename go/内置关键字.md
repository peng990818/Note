...
select语句允许一个Goroutine同时等待多个通信操作（发送或接收）中的一个完成。

### 分支的随机化

select语句本身会被编译为selectgo调用。这与普通的多个if分支不同。selectgo则用于随机化每条分支的执行顺序，普通多个if分支的执行顺序始终是一致的。
...
将未唤醒的sudog从channel的队列中移除是为了清理和防止资源泄漏，确保channel操作的正确性和系统的稳定性。

1. 防止资源泄漏：每个sudog都表示一个等待操作的goroutine，它在等待被唤醒。如果在select语句中，goroutine被唤醒去执行了其他的channel操作，而不是当前channel上的操作，那么sudog需要从该channel的等待队列中移除，以防止后续操作中不必要的资源占用。
2. 保持队列的一致性：channel的等待队列需要保持一致性，以确保后续操作能够正确地处理。如果不移除未唤醒的sudog，这些sudog仍然存在于队列中，可能会导致channel操作的错误和不一致。
3. 确保正确的调度和执行：在select语句中，只有一个case会被选择执行。因此，当goroutine被唤醒后，其他未被选择的channel操作需要被撤销，未唤醒的sudog自然也应该从channel的等待队列中移除，确保只有真正需要被唤醒的goroutine被调度执行。
4. 防止重复唤醒：如果不及时移除未唤醒的sudog，一旦条件触发，它们可能会被错误地再次唤醒，导致一个goroutine被多次唤醒，这是不符合预期的操作，会引发逻辑错误。
5. 提高系统性能：移除未唤醒的sudog还能减少不必要的队列操作，提高channel的操作性能。较少的sudog存在队列中，意味着后续的channel操作会更快的找到真正需要处理的goroutine，提高整个系统的运行效率。

### 发送数据分支

select的诸多用法本质上还是channel操作
...
带有default的select操作是非阻塞的，在chansend函数中，有对非阻塞操作进行特殊处理和优化。非阻塞操作并且没有关闭并且满了没有剩余空间，发送一定失败。

当我们在没有配合for循环使用select时，需要对发送失败进行处理。
...
如果没有default，程序可能会因为死锁而发生panic。

go编译器本质上会对0个case和一个case的情况进行编译优化，不会被翻译成selectgo，在只有一个分支的情况下，select和if没有区别，这种优化会消除只有一个或0个case情况调用selectgo带来的性能开销。
