...
RAID：一种将多个物理磁盘组合成一个或多个逻辑单元以提高数据冗余和性能的技术

| 模式   | 特性                                             | 优点                                                         | 缺点                                                         | 使用场景                                                     |
| ------ | ------------------------------------------------ | ------------------------------------------------------------ | ------------------------------------------------------------ | ------------------------------------------------------------ |
| RAID 0 | 数据在多个磁盘上条带化（分块）写入，增加读写速度 | 高性能，读写速度显著提升                                     | 没有冗余，一块磁盘故障会导致所有数据丢失，单磁盘故障概率较高。 | 需要高性能但不需要数据冗余的场合                             |
| RAID 1 | 数据在两个磁盘完全镜像存储。                     | 高冗余性，一块磁盘故障不会导致数据丢失。读取速度略高于RAID0， | 存储效率低（总容量为一半），写入速度略慢。                   | 需要高数据安全性和冗余的场合，如关键数据存储、操作系统盘。其次对处理日志记录和类似工作负载的服务器很好，因为顺序写入不需要许多底层磁盘就能执行的很好，它是需要冗余但只有两个硬盘驱动器的低端服务器的典型选择。 |
| RAID5  | 数据条带化存储，并在每个磁盘上分配奇偶校验信息。 | 较好的性能和冗余性，能够在一块磁盘故障时恢复数据。可以从奇偶校验块重建数据。 | 写入速度较慢，尤其是小文件写入，重建时间较长。               | 需要平衡性能、容量和冗余的场合，如文件服务器、应用服务器。当一块盘出现故障时 |
|        |                                                  |                                                              |                                                              |                                                              |
